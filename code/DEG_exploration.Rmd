...
custom_volcano <- function(DEG_results, alpha, log2FC, top_exp = 10){
  ...
  top_exp_genes <- top_exp_genes[c(1:top_exp, (length(top_exp_genes) - top_exp):length(top_exp_genes))]
  temp[, gene_label := ifelse(external_gene_name %in% top_exp_genes, external_gene_name, "")]
  # # Select highest positive FC
  # temp[1:top_exp, gene_label := external_gene_name]
  # # Select highest negative FC
  # temp[(nrow(temp) - top_exp):nrow(temp), gene_label := external_gene_name]
  # # If gene has not a valid padj (i.e. NA), it will be ignored
  # temp[, gene_label := ifelse(alpha == FALSE, "", gene_label)]
  # Make plot
