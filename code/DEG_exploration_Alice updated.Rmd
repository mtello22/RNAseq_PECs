...
custom_volcano <- function(DEG_results, alpha, log2FC, top_exp){ 
  # DESeq2 will make some adjusted pvalues NA
  num_na <- sum(is.na(DEG_results$padj))
  
  # Omitting NA results and categorizing each gene
  temp <- data.table(na.omit(DEG_results))
  
  # Categorizing upregulated, downregulated, and non-significant genes
  temp[, DEG := "No"]
  ...
  top_exp_genes <- temp[DEG %in% c("Upregulated", "Downregulated"), external_gene_name]
  top_exp_genes <- top_exp_genes[c(1:top_exp, (length(top_exp_genes) - top_exp + 1):length(top_exp_genes))]

  # Label the top upregulated and downregulated genes
  ...
  ggplot(temp, aes(x = log2FoldChange, y = -log10(padj), color = DEG)) +
    ...
    scale_color_manual(values = c("Downregulated" = "blue", "Upregulated" = "red", "No" = "darkgray"), 
                       labels = c(paste("Downregulated (", as.character(sum(temp$DEG == "Downregulated")), ")", sep = ""), 
                                  paste("Upregulated (", as.character(sum(temp$DEG == "Upregulated")), ")", sep = ""), 
                                  paste("Not Sig. (", as.character(sum(temp$DEG == "No") + num_na), ")", sep = ""))) +
