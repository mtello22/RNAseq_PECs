...
custom_volcano <- function(DEG_results, alpha, log2FC){
  ...
  # Define the DEG status based on the criteria
  temp[, DEG := "No"]  # Default to gray
  temp[, DEG := ifelse(log2FoldChange < -log2FC & padj < alpha, "Downregulated", DEG)]
  temp[, DEG := ifelse(log2FoldChange > log2FC & padj < alpha, "Upregulated", DEG)]
  
  # Set the order of factors to ensure proper color assignment
  temp$DEG <- factor(temp$DEG, levels = c("Downregulated", "Upregulated", "No"))
  ...
  ggplot(temp, aes(x = log2FoldChange, y = -log10(padj), color = DEG)) +
    ...
    scale_color_manual(values = c("Downregulated" = "blue", "Upregulated" = "red", "No" = "darkgray"), 
                       labels = c(paste("Downregulated (", as.character(table(temp$DEG)["Downregulated"]), ")", sep = ""), 
                                  paste("Upregulated (", as.character(table(temp$DEG)["Upregulated"]),")", sep = ""), 
                                  paste("Not Sig. (", as.character(table(temp$DEG)["No"] + num_na), ")", sep = ""))) +
